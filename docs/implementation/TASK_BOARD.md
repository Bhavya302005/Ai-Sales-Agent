...
| P12 | DONE | Implement unified Hirevia design system Admin login portal, credentials validation, restricted registration, and warm paper aesthetic |
...
| P14 | DONE | iPhone-installable PWA has Apple/manifest PNG assets, standalone metadata, safe non-caching service-worker lifecycle, install guidance, verified production responses, and an ngrok launch script; live tunnel awaits local account-token configuration |
